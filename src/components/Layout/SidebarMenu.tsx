...
export type MenuItem = {
  title: string;
  href?: string; // optional for toggle items
  order: number;
  children?: MenuItem[];
  active?: boolean;
};
...
export default function SidebarMenu() {
  ...
  const ordered = useMemo(() => {
    const sortDeep = (items?: MenuItem[]): MenuItem[] => {
      ...
      list.sort(byOrder);
      return list.map((i) => ({ ...i, children: sortDeep(i.children) }));
    };
    return sortDeep(menu);
  }, [menu]);

  const renderLevel = (items: MenuItem[], level = 0) => (
    <div
          ...
          >
            <Link
              ...
              className={`accordion-link ${
                normalizedHref === pathname ? "active" : ""
              }`}
            >
              {item.title}
            </Link>
  ...
  return (
    // Start Responsive Sidebar Menu Area
    <div
      ...
      <div className="offcanvas-body">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit="exit"
          >
            {/* Accordion root (keeps classes as provided) */}
            {renderLevel(ordered)}
          </motion.div>
        </AnimatePresence>

        {/* Others Option (classes preserved) */}
